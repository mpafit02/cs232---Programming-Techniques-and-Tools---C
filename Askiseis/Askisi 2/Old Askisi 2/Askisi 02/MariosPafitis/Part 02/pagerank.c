...
//Methods Declaration
int OCalc(int h, int n, int **);
int OiCalc(int, int, int **);
void ICalc(int **, int, int, int **);
void PrCalc(double, int, int **, int *, int **, double *);
int efklidiaDistance(int, double *, double *);
double normPagerank(int, int, double *);
...
int main(int argc, char **argv) {
	...
	char str[100]; // General use character array
	...
	while (fscanf(fp1, "%d %s", &id, name) != EOF) {
		...
		hostnames[hostCounter] = (char *) malloc(strlen(name) * sizeof(char));
		strcpy(hostnames[hostCounter], name);
		hostCounter++;
	}
	hostnames = (char **) realloc(hostnames, hostCounter * sizeof(char *));

	//Cretae malloc for hostgraph, the I, O, Oi table and Pr
	hostgraph = (int **) malloc(hostCounter * sizeof(int *));
	I = (int **) malloc(hostCounter * sizeof(int *));
	Oi = (int **) malloc(hostCounter * sizeof(int *));
	for (i = 0; i < hostCounter; i++) {
		hostgraph[i] = (int *) malloc(hostCounter * sizeof(int));
		I[i] = (int *) malloc(hostCounter * sizeof(int) + 1);
		Oi[i] = (int *) malloc(hostCounter * sizeof(int));
		O = (int *) malloc(hostCounter * sizeof(int));
		Pr = (double *) malloc(hostCounter * sizeof(double));
...
//	}
	...
	// Calculate Pr
	PrCalc(D, n, I, O, Oi, Pr);

	// Calculate Norm Page Rank
	for (i = 0; i < n; i++) {
		normPr[i] = normPagerank(i, n, Pr);
	}
	// Print Pr
 ...
 * 	@bug No known bugs.
 */
void ICalc(int **I, int h, int n, int **hostgraph) {
	int i;
	I[h][0] = 0;
	for (i = 0; i < n; i++) {
		if (hostgraph[i][h] != 0) {
 ...
 * 	@bug No known bugs.
 */
void PrCalc(double d, int n, int **I, int *O, int **Oi, double *Pr) {
	int i, j, finished = 0;
	int counter = 0;
	double PrOld[n];
	while (!finished) {
		...
		counter++;
		finished = 1;
		if (efklidiaDistance(n, PrOld, Pr) == 0) {
			finished = 0;
//			// Print to see the Iterations
//			printf("\n\nIteration: %d", counter);
//			for (i = 0; i < n; i++) {
 ...
 * 	@bug No known bugs.
 */
int efklidiaDistance(int n, double *PrOld, double *Pr) {
	int i, j;
	double result, sum = 0;
	for (i = 0; i < n; i++) {
 ...
 * 	@bug No known bugs.
 */
double normPagerank(int hi, int n, double *Pr) {
	int i;
	double sum = 0;
	for (i = 0; i < n; i++) {
		sum += *(Pr + i);
